...
# --> need to update code; upload datasets, etc.
...
if(!dir.exists(file.path("..", "Manuscript", "Images", "other"))) {dir.create(file.path("..", "Manuscript", "Images", "other"))}
############################################################
# setup
############################################################
...
map1 <- map0 + 
  #monitoring area
  geom_sf(data = monitoring_area_shp2, aes(fill = "Monitoring Area"), 
          inherit.aes = F,
          lwd = 0.1, alpha = 0.1) + 
  
  #jittered ACT locations
  geom_point(data = act_shp_jit, aes(shape="ACT Cohort", y=lat_jit_deg, x=long_jit_deg),inherit.aes = F,
          alpha=0.1, #0.2
          size=0.5) +
  #routes 
  geom_sf(data=routes_shp, aes(col = Route), inherit.aes = F, alpha=0.7) +
  
  #monitoring stops
  geom_sf(data = mm_shp, aes(col = Route), inherit.aes = F, 
          size=1.5 #2
          ) + 
  #colocations
  geom_sf(data = filter(mm_shp, Kind == "AQS Colocation"), aes(shape= "AQS Colocation"), inherit.aes = F, size=1.7) + 
  ...
  geom_sf(data = monitoring_area_shp2, inherit.aes = FALSE, alpha=0, lwd = 0) +
...
ggsave(file.path("..", "Manuscript", "Images", "other", "monitoring_map.jpg"), width = 7.5, height = 10)
...
### --> compare BH vs RH data variability (by design, # days, 1 season, # days...) 
### --> but see same patter in CA NOx data?
...
### --> also show campaign-reference R2s
...
campaign_descriptions %>%
  ...
  labs(y = "R2", # MSE-Based R2
       x = "Design")

ggsave(file.path(image_path, #"SI", 
                 "r2.png"), width = 6, height = 4)
