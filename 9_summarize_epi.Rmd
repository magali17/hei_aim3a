...
# --> ERROR???!
#TEST - dt from plasmid
plasmid_selected_campaigns <- readRDS("Output/UK Predictions/cohort/KP/selected_campaigns_v2.rda") %>%
  mutate(model2 = as.numeric(as.character(gsub("mb_", "", model_id))))%>%
  arrange(model2)

summary(plasmid_selected_campaigns$model2)


# WHAT WE SENT TO KP
#predictions_20230215 <- readRDS("Output/UK Predictions/cohort/KP/predictions_2023-02-15.rda") %>%
predictions_20230216 <- read.csv("Output/UK Predictions/cohort/KP/predictions_2023-02-16.csv") %>%
  mutate(model2 = as.numeric(as.character(gsub("mb_", "", model)))) #%>% arrange(model2)

## WHAT WE SENT TO KP
length(unique(predictions_20230215$model2))
summary(unique(predictions_20230215$model2))
# summary(unique(predictions_20230216$model2))

# looks OK...hm
#unique(plasmid_selected_campaigns$model2)[!unique(plasmid_selected_campaigns$model2) %in% unique(predictions0$model2)]

# WHAT WE GOT BACK
issue17 <- readRDS(file.path("data", "issue_17", "issue_017v2_01262023.rda"))%>%
  mutate(model2 = as.numeric(as.character(gsub("mb_", "", model))))%>%
  arrange(model2)
## WHAT WE GOT BACK
length(unique(issue17$model2))
summary(unique(issue17$model2))

# correct. # subjects (vs locations)
# length(unique(issue17$study_id))
...
campaign_descriptions <- readRDS(file.path(
  #"Output", 
  dt_path, "Selected Campaigns",
  
  # --> TEMP
  #"old", "20230126 full has 278 sites", #"selected_campaigns2.rda"
  "selected_campaigns_v2.rda")) %>%
...
# # TEST
# --> these are totally off??
# summary(campaign_descriptions$model_id2)
# summary(unique(cs$model_id2))

# missing_ids <- unique(cs$model_id)[!unique(cs$model_id) %in% campaign_descriptions$model_id]
# missing_campaigns <- campaign_descriptions %>% filter(model_id %in% missing_ids)
...
# --> calculate above/below median by year?  group_by(variable, year2)
...
t1read(filter(pop), t1_labels) %>%
  table1(~ visit_age + male + year2 + casi_irt + apoe + degree + #race_white + 
           exposure_PNC, #  | high_exposure_PNC, 
         data=.)
...
model_coefs %>%
    filter(grepl("Total", variable),
           #design != "All Data",
           grepl("Fewer Total|Seasons", design)
           ) %>%
...
ggsave(file.path(image_path, "other", "presentation", "beta_total_a.png"), width = 8, height = 6)
