...
# --> NOTE, MAY NEED TO CHANGE "Total" to 10-420 / primary analysis???
...
# version levels
model_perf_other0 <- readRDS(file.path(dt_path, "other_designs_model_eval.rda")) %>%
  filter(!grepl("balsea", model)) 

cw_other <- read.csv(file.path(dt_path, "other_designs_model_cw.csv"))

visit_count <- seq(2,22, 2)
fewer_hr_lvls <- unique(cw_other$version)[str_detect(unique(cw_other$version), "business|rush")] %>% str_to_title()
site_type_lvls <- paste("H", rev(visit_count), "L", visit_count)

model_perf_other <- model_perf_other0 %>%
  left_join(cw_other) %>%
  mutate(adjusted = ifelse(grepl("temp adj", version, ignore.case=T), "Temporally Adjusted", 
                           "Not Temporally Adjusted"), 
                           #ifelse(!grepl("temp adj", version, ignore.case=T) & design== "Fewer Hours", "Not Temporally Adjusted", NA))
        version = gsub(" Temp Adj", "", version, ignore.case=T),
        
        version = str_to_title(version),
         version = factor(version, levels=c(fewer_hr_lvls, site_type_lvls)),
...
annual_trainint_set <- readRDS(file.path(dt_path, "annual_training_set.rda"))
...
annual_avgs <- annual_trainint_set %>%
  ...
  # --> ???
  filter(grepl("pnc_|total_|10_100", variable),
         !(version=="4" & design=="balanced seasons")) %>%
...
ggsave(file.path(image_path, "SI", "site_averages.png"), width = 6, height = 5)
...
# lapply(c(main_pollutants_labels, sensitivity_pollutant_labels), function(x) {
...
campaign_descriptions %>%
  ...
  geom_boxplot(aes(col=analysis))+
  facet_grid(variable~design, scales = "free", switch = "both") + 
  labs(y = "R2", x = "Design", col="Analysis")

ggsave(file.path(image_path, "SI", "r2.png"), width = 6, height = 8)

```

### other designs 

#### --> LS: "show temporal adj as scatterplot – so we can see that there’s never a campaign that is better" 

```{r}
print("Distribution of UK-PLS campaign performances. 30 campaigs per version")

# same as below, for presentations
model_perf_other %>%
  filter(variable == "ns_total_conc") %>% 
  pivot_longer(cols = c(MSE_based_R2, RMSE)) %>%
  filter(name=="MSE_based_R2") %>%
  
  ggplot(aes(x=version, y=value, col=adjusted)) +
  facet_grid(~design, scales = "free", switch = "both", space = "free_x") +
  geom_hline(data=select(filter(t2, design=="All Data"), variable, Q50), aes(yintercept = Q50), linetype=2, alpha=0.5) + 
  geom_boxplot() +
  scale_y_continuous(breaks = seq(0.3, 0.65, .05)) +
  labs(y="R2", x="Design", col=NULL)

ggsave(file.path(image_path, "other", "presentation", "other_designs_model_performances.png"), width = 10, height = 5)
...
model_perf_other %>%
  ...
  geom_boxplot() +
  labs(y="Campaign Performance")

ggsave(file.path(image_path, "SI", "other_designs_model_performances.png"), width = 14, height = 8)
...
#        x = "Design")
# 
# ggsave(file.path(image_path, "predicted_exposure.png"), width = 6, height = 4)
# 
# # SI
# cs %>%
#   filter(variable %in% sensitivity_pollutant_labels) %>%
...
#    geom_boxplot(aes(col=analysis)) + 
#   facet_grid(variable~design, scales = "free", switch = "both") + 
#   labs(y = "Predicted Exposure", x = "Design", col="Analysis")
# 
# ggsave(file.path(image_path, "SI", "predicted_exposures2.png"), width = 6, height = 8)
...
lapply(sensitivity_pollutant_labels, function(x) {
  ...
  p <- comp_exp %>%
  filter(variable== x#main_pollutants_labels
         ) %>%
    ...
    labs(x = "All Data Exposure Prediction", y= "Sampling Design Exposure Prediction", col="Campaign")

  if(grepl("Total|10-420", x)) {
    print(p)
    ggsave(file.path(image_path, paste0("predicted_exposure_",x ,".png")), width = 5.5, height = 7)
    } else{
      p <- p + labs(title = x)
      print(p)
      ggsave(file.path(image_path, "SI", paste0("predicted_exposure_",x ,".png")), width = 5.5, height = 7)
    }
...
ggsave(file.path(image_path, #"SI", 
                 "prediction_correlations.png"), width = 6, height = 4)
...
corr_table %>%
  ...
  labs(y = "Correlation (R)", x = "Design", col="Analysis")

ggsave(file.path(image_path, "SI", "prediction_correlations2.png"), width = 6, height = 8)
...
# x = p_list[2]
lapply(p_list, #all_pollutants_labels, 
       function(x){ 
      ...
      geom_pointrange(aes(y=est, ymin=lower, ymax=upper), size=0.5, alpha=0.8) +
      labs(title=x, col = "Statistically Significant", x="Campaign", y = "Beta Estimate")
      
      if(grepl("10-420 nm|Total", x)) {
        p <- p + facet_wrap(~design+version) 
        print(p)
        ggsave(file.path(image_path, paste0("beta_", x, ".png")), width = 8, height = 8)
        } else {
        p <- p + facet_wrap(~design+version, ncol = 2) 
        print(p)
        ggsave(file.path(image_path, "SI", paste0("beta_", x, ".png")), width = 6, height = 8)}
      })

```


side note - correlations vs estimates
```{r}
test <- model_coefs %>%
  inner_join(select(corr_table, model, r))
...
ggplot(aes(x=campaign, shape=significant,
           col=r
...
geom_hline(data=filter(true_betas, variable==x), aes(yintercept = est)) +
geom_pointrange(aes(y=est, ymin=lower, ymax=upper), size=0.5, alpha=0.8) +
labs(title=x, col = "Correlation (R) With All-Data Prediction", x="Campaign", y = "Beta Estimate") + facet_wrap(~design+version) 
...
model_coefs %>%
  ...
  scale_y_continuous(sec.axis = sec_axis(trans=~(.-tot_pnc)/-tot_pnc, name="% Difference", labels = scales::percent), limits = y_lims) +
    geom_label(data=mutate(filter(true_betas, grepl("Total|10-420", variable)), design="All Data" ) %>% mutate(design= factor(design, levels= levels(model_coefs$design))), aes(x=1, y=est, label = round(tot_pnc, 3)), #col=uw_purple
               ) +
  labs(y = "Beta Point Estimate", x = "Design")

ggsave(file.path(image_path, "beta_boxplots_main.png"), width = 6, height = 4)

# SI

lapply(c("10-420 nm", 
         "10-100 nm", "20-1,000 nm"), function(x) {
  ...
  p <- model_coefs %>%
    ...
    geom_label(data=filter(true_betas, variable==x) %>% mutate(design="All Data", design= factor(design, levels= levels(model_coefs$design))),
               aes(x=1, y=est, label = round(est, 3))) +
  labs(y = "Beta Point Estimate", x = "Design")
...
ggsave(file.path(image_path, "SI", paste0("beta_boxplots.png")), width = 8, height = 10)
