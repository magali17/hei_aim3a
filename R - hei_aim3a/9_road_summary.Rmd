...
t1_a <- cw %>%
  distinct(#spatial, 
    version, design, visits, adjusted) %>% 
...
all_predictions %>%
  filter(model != ref_model_coefs$model,
         # route design has more options
         visits %in% main_visits
         ) %>%
...
ggsave(file.path(image_path, "SI", "predicted_ufp_309_sites.png"), width = 22, height = 8)
...
#--> update figs & dimensions of saved figs
...
model_eval %>%
  # route designs have more options
  filter(visits %in% main_visits,
         # ignore larger spatial clusters here
         !grepl("_c2_", model)) %>%
  mutate(visits = paste(visits, "visits")) %>% 
  
  group_by(version, design, visits, cluster_type) %>%
...
# --> for SI: plot comparing cluster 1 vs 2 - show results are similar




# --> for SI: same plot just for routes - show that get similar results w/ increasing numbers
...
# --> repeat fig for route w/ more visit options??
