...
model_eval2 %>%
  ...
  group_by(design, version, time_selection, visit_label) %>%  
  summary_table(var = "MSE_based_R2") %>% 
  
  ggplot(aes(fill=design, x=version)) +
...
```{r}
# # Define a color palette that visually increases in intensity
# cb_palette <- c("Balanced" = "#56B4E9",   # Sky Blue
#                 "Random Clusters" = "#009E73",   # Bluish Green
#                 "Sensible Clusters" = "#F0E442",   # Yellow
#                 "Unsensible Clusters" = "#E69F00", # Orange
#                 "Road Type" = "#D55E00")  # Vermillion

health_plot2 <- function(dt=model_coefs,
                         ref_stationary_restimate=tot_pnc){
...
model_coefs_reduced_model %>%
  keep_hei_results() %>%
  add_labels() %>%
  health_plot2(ref_stationary_restimate = tot_pnc_reduced)

ggsave(file.path(image_path, "HEI Report", paste0("betas.png")), width = 10, height = 8)
...
ggsave(file.path(image_path, "HEI Report", "Appendix", paste0("betas_extended_model.png")), width = 10, height = 8)
