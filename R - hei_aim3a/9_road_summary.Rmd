...
#       design = case_when(
#         design=="balanced" ~ "balanced", # "balanced segments"
#         design=="unbalanced" ~ "random segments", #"unbalanced segments"
#         design=="random" ~ "random clusters", # "unbalanced random clusters"
#         design=="sensible" ~ "sensible clusters", # unbalanced sensible clusters
#         design=="unsensible" ~ "unsensible clusters", # unbalanced unsensible clusters
#         design=="road_type" ~ "road type", #unbalnaced road type
#         design=="route" ~ "route", # balanced routes
#         # design=="route" ~ "balanced routes",
#         # design=="balanced" ~ "balanced segments",
#         # design=="unbalanced" ~ "unbalnaced segments",
...
# COMMON VARIABLES
##################################################################################################
main_designs <- design_levels #%>%  str_subset(., "Sensible|Unsensible|Road Type", negate = T)
...
model_eval %>%
  add_labels() %>% #distinct(visits)
  keep_main_results(designs_to_keep = c("Balanced Routes", "Balanced Segments"), visits_to_keep = c(4, 12, 20)) %>%
  plot_values(summary_var = "MSE_based_R2", y_label = expression("Exposure Model" ~ R[MSE]^2),
              add_dahsed_line = T, ref_dt = stationary_r2_mse_ref)
...
health_plot <- function(dt, #model_coefs
  ...
  p <- dt %>%
   ...
   #geom_text(inherit.aes = F, x=1, y=ref_stationary_restimate*1.1, label="Reference", fill= "white",show.legend = F, size=5) +
    ...
    geom_boxplot(position = "dodge", stat="identity", aes(ymin=Q10, lower=Q25, middle=Q50, upper=Q75, ymax=Q90)) +
    scale_y_continuous(sec.axis = sec_axis(trans=~(.-ref_stationary_restimate)/-ref_stationary_restimate, 
                                           name= NULL, #"Health Point Estimate % Difference", 
...
lapply(c(4, 12), function(v){
  ...
  annotate_figure(left = "Health Point Estimate", 
                  right = "Health Point Estimate % Difference")

if(v==12){ggsave(file.path(image_path, "betas.png"), width = 10, height = 8)} 

if(v==4){ggsave(file.path(image_path, "SI", paste0("betas_", v, "_visits.png")), width = 10, height = 8)}

})

```

# --> update plot? color by visits vs balanced design; combine main & reduced model

```{r}
# routes
model_coefs %>%
  add_labels() %>% 
  keep_main_results(designs_to_keep = c("Balanced Routes", "Balanced Segments"), 
                    visits_to_keep = c(4, 12, 20)) %>%
  
  health_plot(dt=., ref_stationary_restimate=tot_pnc) + 
  labs(y = "Health Point Estimate") + 
  # add second y-axis label to this plot
  scale_y_continuous(sec.axis = sec_axis(trans=~(.-tot_pnc)/-tot_pnc,
...
if(v==12){ggsave(file.path(image_path, "HEI Report", "betas.png"), width = 10, height = 8)} 

if(v==4){ggsave(file.path(image_path, "HEI Report", "Appendix", paste0("betas_", v, "_visits.png")), width = 10, height = 8)}
