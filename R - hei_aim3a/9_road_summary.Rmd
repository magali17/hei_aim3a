...
major_roads <- osmdata::opq(bbox_vector) %>%
...
r3$table %>%
  ...
  write.csv(file.path(image_path, "r2_mse.csv"), row.names = F)
...
model_eval %>%
  ...
  filter(visits %in% 12,
         ...
         !grepl("Sensible|Unsensible", design),
...
ggsave(file.path(image_path, "SI", paste0("r2_mse_routes.png")), width = 8, height = 6)
...
df_merged %>%
  # drop random noise
  filter(random_adjustment=="Main") %>%
  
  ggplot(aes(x=MSE_based_R2_no_adjustment, y= MSE_based_R2_other, col=design, 
             #shape=random_adjustment, linetype=random_adjustment
             )) + 
...
model_eval %>%
  ...
  group_by(across(all_of(c("design", "version", "time_selection", "visit_label", "name")))) %>%
  summary_table(var = "value") %>%
  mutate(name = ifelse(name=="MSE_based_R2", "MSE-based\nR2 (Main)", "Regression-based\nR2")) %>%
...
ggsave(file.path(image_path, "SI", "r2_regression.png"), width = 9, height = 6)
...
```{r}
# reduced models
model_coefs_reduced_model_table <- model_coefs_reduced_model %>%
  add_labels() %>%
  keep_main_results() %>%
  mutate(design = gsub("\n", " ", design)) %>%
  group_by(design, time_selection, version, visit_label) %>%
...
model_coefs_reduced_model_table %>%
  mutate_if(is.numeric, ~round(., 3)) %>%
  write.csv(., file.path(image_path, "betas_reduced_health_model.csv"), row.names = F)


# extended models
model_coefs_extended_model_table <- model_coefs %>%
  add_labels() %>%
  keep_main_results() %>%
  mutate(design = gsub("\n", " ", design)) %>%
  group_by(design, time_selection, version, visit_label) %>%
...
model_coefs_extended_model_table %>%
  mutate_if(is.numeric, ~round(., 3)) %>%
  write.csv(., file.path(image_path, "betas_extended_health_model.csv"), row.names = F)
...
lapply(c(4, 12), function(v){
  ...
  extended_health_model <- model_coefs %>%
    add_labels() %>%
    keep_main_results(visits_to_keep = v) %>%
    mutate(visit_label = paste0("Primary Health Model (", visit_label, ")")) %>%
    health_plot(dt=., 
...
if(v==12){ggsave(file.path(image_path, "betas.png"), bg="white", width = 11, height = 11)} 

if(v==4){ggsave(file.path(image_path, "SI", paste0("betas_", v, "_visits.png")), bg="white", width = 11, height = 11)}
...
plot_point_ci <- function(beta_data, reference_data){
  
  beta_data %>%
  add_labels() %>%
  keep_main_results(visits_to_keep = 12,
                    versions_to_keep = c("No Adjustment",
                                          "Plume Adj"#,
                                         #"Underwrite Temporal Adj"#,
                                         #"Fixed Site Temporal Adj"
                                         )

                    ) %>% 
   mutate(design=gsub("\n", " ", design)) %>%
  
  ggplot(aes(x=campaign, shape=version, col=version)) +
  facet_wrap(~design+time_selection, ncol = 2) + 
  
  geom_hline(yintercept = 0, linetype=2, alpha=0.2, col="red") +
  geom_rect(inherit.aes = F, data=reference_data, aes(ymin = lower, ymax = upper, xmin=-Inf, xmax=Inf), alpha=rectangle_alpha, fill=ref_model_col) +
  geom_hline(data=reference_data, linetype=2, aes(yintercept = est), col=ref_model_col) +
  geom_pointrange(aes(y=est, ymin=lower, ymax=upper), size=0.4,
                  position=position_dodge(width=0.7)) +
  scale_x_continuous(breaks = c(1, seq(10,30,10))) +
  labs(x="Campaign", 
       y= expression(beta[m] ~ " (95% CI)"),
       #y = "Health Estimate (95% CI)", 
       shape=NULL, col=NULL)
...
# reduced model
plot_point_ci(model_coefs_reduced_model, ref_model_coefs_reduced)

ggsave(file.path(image_path, "SI", "beta_point_ci_reduced.png"), width = 8, height = 10)

# extended model
plot_point_ci(model_coefs, ref_model_coefs)

ggsave(file.path(image_path, "SI", "beta_point_ci_extended.png"), width = 8, height = 10)
