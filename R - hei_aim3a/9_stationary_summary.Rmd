...
r2_plot <- function(dt=model_eval, y_variable, facet_variable = "10-420|Total", plot_y_label="PNC") {
  ...
  dt %>%
    ...
    scale_y_continuous(sec.axis = sec_axis(trans=~./ref_r2, name=paste(plot_y_label, "Exposure Model R2 (% of Reference Model)"), labels = scales::percent), position="right") +
...
lapply(levels(refs1$variable), function(x) {r2_plot_multifacet(y_variable= "MSE_based_R2", facet_variable=x)}) %>%
  ...
  annotate_figure(left = "Exposure Model R2 (% of Reference Model)",
                  right = "Exposure Model R2",
                  bottom = "Design")
...
lapply(levels(refs1$variable), function(x) {r2_plot_multifacet(y_variable= "reg_based_R2", facet_variable=x)}) %>%
  ggarrange(plotlist = ., ncol = 1, common.legend = T, legend = "bottom") %>%
  annotate_figure(left = "Exposure Model R2 (% of Reference Model)",
                  right = "Exposure Model R2 (Regression-Based)",
                  bottom = "Design")
...
beta_summary_plot <- function(dt, true_betas., variable.="Total|10-420", facet_label="Reduced Model") {
  ...
  scale_y_continuous(sec.axis = sec_axis(trans=~(.-true_betas.$est)/-true_betas.$est, 
                                         name= NULL,# "Health Point Estimate (% Different from Reference)", 
...
# --> TO DO: COMBINE reduced & extended 
reduced_models <- beta_summary_plot(dt=model_coefs, true_betas.=true_betas)
extended_models <- beta_summary_plot(dt=model_coefs_sensitivity, true_betas.=true_betas_sensitivity, facet_label = "Primary Model")
...
ggarrange(reduced_models,  extended_models, ncol = 1, common.legend = T, legend = "bottom") %>%
  annotate_figure(right = "Health Point Estimate",
                  left = "Health Point Estimate (% Different from Reference)", 
                  bottom = "Design")
ggsave(file.path(image_path, "beta_boxplots_main_reduced.png"), width = 12, height = 6)
...
# # SI - NO2
# beta_summary_plot(dt=model_coefs, true_betas.=true_betas, variable. = "NO2")
# ggsave(file.path(image_path, "beta_boxplots_no2.png"), width = 12, height = 6)




# SI - other PNC measures
# x="10-420 nm" #all_pollutants_labels[3]

# x= "NO2 (ppb)"
lapply(c("10-420 nm", "10-100 nm", "20-1,000 nm", "NO2 (ppb)"), 
       function(x) {
  
  true_betas2 <- true_betas_sensitivity %>% 
