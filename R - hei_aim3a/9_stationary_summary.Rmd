...
model_coefs <- model_coefs0 %>% 
  filter(model %in% model_eval$model) %>%
  label_pollutants() %>% label_designs() %>%
   mutate(
    ref = ifelse((design == "Fewer Visits" & version == "12") |
                   (design == "Fewer Seasons" & version == "4") |
...
model_coefs %>%
  ...
  #                    ) +
  scale_y_continuous(sec.axis = sec_axis(trans=~(.-tot_pnc)/-tot_pnc, name="Health Point Estimate (% Different from Reference)", labels = scales::percent),
                     position="right") +
