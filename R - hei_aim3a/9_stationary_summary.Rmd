...
version_levels2 <- setdiff(levels(campaign_descriptions$version), c("Business", "Rush")) %>%
  append(c("Business",
         "Business \nTemporally\nAdjusted",
         "Rush",
         "Rush \nTemporally\nAdjusted"#,
...
# balanced_versions2 <- grep("^H\\d", levels(campaign_descriptions$version), value = T) %>%
#   gsub(" ", "\n", .) %>%
#   gsub("H", "High ", .) %>%
#   gsub("L", "Low ", .)  
...
temp <- data.frame(design=c("Fewer Visits"),
                   ...
                   est=c(r2_ref),
                   version = "6") %>%
  mutate(design = factor(design, levels = levels(campaign_descriptions$design)))
...
campaign_descriptions %>%
  ...
  facet_wrap(~design, scales = "free_x", switch = "x") +
  geom_label(data=temp, aes(y=est, label=label), fill= "white",
             col=uw_purple,
...
model_coefs %>%
  ...
  scale_y_continuous(sec.axis = sec_axis(trans=~(.-tot_pnc)/-tot_pnc, name="Health Point Estimate\n% Difference", labels = scales::percent),
                     position="right") +
    geom_label(data=temp, aes(y=est, label=label), fill= "white",show.legend = F, size=5) +
  geom_label(data=filter(temp, grepl("Reference", label, ignore.case = T)), aes(y=est, label=label), fill= "white",col=uw_purple, show.legend = F, size=5) +
  poster_theme +
  labs(y = "Health Point Estimate        ", x = "Design",
       fill= NULL)

ggsave(file.path(image_path, "other", "presentation", "beta_boxplots_main_large_text.png"), width = 13, height = 8)
