...
##################################################################################################
# site predictions
# site predictions for temporally adjusted designs & others?
other_design_site_predictions <- readRDS(file.path(dt_path, "UK Predictions", 
                                                   #"other_design_predictions.rda"
                                                   "other_design_predictions_20240516.rda"
                                                   )) %>%
  # -> UPDATE here & elsewhere: add pnc_noscreen?
  filter(grepl("nstot_fewhrs|pnc_noscreen", model)) %>%
  left_join(read.csv(file.path(dt_path, "other_designs_model_cw_20240516.csv"))) %>%
  label_pollutants() %>% label_designs() %>%
...
# --> TO DO COMBINE, DECIDE WHAT 
cw0 <- read.csv(file.path(dt_path, "model_cw.csv")) %>%
...
model_eval_other_designs <- readRDS(file.path(dt_path, "other_designs_model_eval_20240516.rda"))
...
comp_exp %>%
  ...
  mutate(prediction_diff = exposure-all_exposure)
