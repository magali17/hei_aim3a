...
if(!dir.exists(file.path(image_path, "other", "qc"))) {dir.create(file.path(image_path, "other", "qc"), recursive = T)}
...
##################################################################################################  
model_covars_reduced <- readRDS(file.path(output_data_path, "model_covars.rda"))

model_covars_extended <- readRDS(file.path(output_data_path, "model_covars_extended.rda"))

# need more complete datset for HEI ("Model 1 - Reduced Model")
...
model_coefs_extended <- readRDS(file.path(output_data_path, "model_coefs_extended.rda")) %>% 
  filter(model %in% unique(cs$model)) %>%
  right_join(select(cw, model, variable, design, version, campaign), by="model") %>%
  label_pollutants() %>% label_designs() %>%
   mutate(
...
exclusion_table <- read.csv(file.path(output_data_path, "exclusion_table2024-07-25.csv")) #exclusion_table2024-05-20.csv
...
ap_age_equivalents <- readRDS(file.path(output_data_path, "model_coefs_extended_all.rda")) %>%
  ...
  mutate(
    ...
    # add CI to age-equivalent AP effect estimate
    ap_age_equivalent_lower = lower_ap/est_age,
    ap_month_equivalent_lower = ap_age_equivalent_lower*12,
    
    ap_age_equivalent_upper = upper_ap/est_age,
...
predictions0 <- readRDS(file.path(dt_path, "UK Predictions", "all_predictions.rda")) %>%
  filter((design %in% c("full", "fewer total stops") |
            design == "fewer hours" & grepl("no2|ns_10_100 ", variable)), #or ns_10_100 
         variable %in% all_pollutants) %>%
  left_join(cw)

# predictions0 %>% filter(location==first(location), variable=="no2", grepl("_01", model), design=="fewer hours")  

# combine predictions
## other designs: unbalanced, temporally adjusted, fixed balanced seasons...
predictions <- readRDS(file.path(dt_path, "UK Predictions", "other_design_predictions_20240516.rda")) %>%
...
# predictions %>% filter(campaign==1, location==first(location)) %>% View()
...
# MEASUREMENT ERROR 
##################################################################################################  
me <- read.csv(file.path(dt_path, "error", "total_bias.csv"))
...
r2_plot <- function(dt=model_eval, y_variable, facet_variable = "10-420|Total", plot_y_label="PNC") {
  ...
  results_plot <- results_table %>%
    ...
    labs(#y =paste(plot_y_label, "Exposure Model R2")  
          y = bquote(.(plot_y_label) ~ "Exposure Model" ~ R[MSE]^2), 
            
         x = "Design", fill=NULL)

  return(list(plot=results_plot,
              table = results_table)
         )
  }


r2_plot(y_variable = "MSE_based_R2")$plot  
                      
ggsave(file.path(image_path, "r2.png"), width = 8, height = 7)


r2_plot(y_variable = "MSE_based_R2")$table %>%
  select(design, version, Min, Q10, Q25, Q50, Q75, Q90, Max) %>%
  arrange(design) %>%
  mutate_if(is.numeric, ~round(., 2)) %>%
  write.csv(file.path(image_path, "HEI Report", "Appendix", "r2_mse.csv"), row.names = F)


################################################################################
# SI Plots - sensitivity analyses

# Updated: does not have % scale on right side b/c of how its plotted
r2_plot_multifacet <- function(dt = model_eval, y_variable) {
  ...
  dt %>%
    ...
    #                                        labels = scales::percent,
    #                                        breaks = seq(0, 1, .2)),
    #                    position = "left", limits = c(0, 0.8)) +
    labs(x = NULL,
         fill = NULL)
}

r2_plot_multifacet(y_variable= "MSE_based_R2") + 
  labs(y=expression("Exposure Model" ~ R[MSE]^2))

ggsave(file.path(image_path, "SI", "r2_sensitivity.png"), bg="white", width = 12, height = 12)


# regression-based R2
r2_plot_multifacet(y_variable= "MSE_based_R2") + 
  labs(y=expression("Exposure Model" ~ R[Regression]^2))

ggsave(file.path(image_path, "SI", "r2_reg_based_sensitivity.png"), bg="white", width = 12, height = 12)
...
```{r, eval=F}
# setting this to eval=F since function was updated since this and facet_variable is no longer a parameter

# no NO2
lapply(c("10-420 nm (Primary)", "10-100 nm", "20-1,000 nm"), function(x) {r2_plot_multifacet(y_variable= "MSE_based_R2", facet_variable=x)}) %>%
  ggarrange(plotlist = ., ncol = 1, common.legend = T, legend = "bottom") %>%
  annotate_figure(right = "% of Reference Model",
                  left = text_grob(expression("Exposure Model" ~ R[MSE]^2), rot = 90),
                  bottom = "Design")

ggsave(file.path(image_path, "HEI Report", "Appendix", "r2_sensitivity.png"), bg="white", width = 12, height = 11)
...
ggsave(file.path(image_path, "HEI Report", "Appendix", "prediction_correlations.png"), width = 12, height = 14)
...
t1read(pop, t1_labels#, render.continuous.default = my.render.cont
  ...
  table1(~ visit_age + male + degree + race_white +  nses_z_cx + casi_irt + exposure | exposure_group, data=.)

```

## HEI Report / Reduced Health Models 

Same table as above, but for the reduced model, which is used for the HEI report and has fewer adjustment variables (i.e., less missingness)

```{r}
pop_reduced_health_model <- cs0 %>%
  
  drop_na(all_of(model_covars_reduced)) %>%
...
names(pop_reduced_health_model) <- gsub("\\ .*|\\,.*", "", names(pop_reduced_health_model))

 # Reduced Health Model
t1read(pop_reduced_health_model, t1_labels#, render.continuous.default = my.render.cont
         ) %>%
  table1(~ visit_age + male + degree + race_white +  nses_z_cx + casi_irt + exposure | exposure_group, data=.)
...
p_list <- unique(true_betas_extended$variable) 
...
lapply(p_list, function(x){ 
      ...
      p <- model_coefs %>%
        ...
        ggplot(aes(x=campaign, shape=adjusted)) +
        geom_hline(yintercept = 0, linetype=2, alpha=0.2) +
      ...
      labs(x="Campaign", y = "Health Estimate (95% CI)", shape=NULL, col=NULL)
        ...
        ggsave(file.path(image_path, "SI", paste0("beta_", x, "_reduced_models.png")), width = 12, height = 8)
      })
...
y_lims <- c(config$health_plot$min_y_ci, config$health_plot$max_y_ci)


y_lims_pct <- seq(-2,2,1)


# dt=model_coefs_extended
summarize_betas <- function(dt, grouping_vars = c("design", "version", "variable")) {
  dt %>%
    filter(grepl(main_pollutants_labels, variable)) %>%
    mutate(
      ref_est = est[grepl("_all_", model)],
    ...
    summarize(
      n = n(),
      min = min(est),
      Q10 = quantile(est, 0.10),
      Q25 = quantile(est, 0.25),
      median = median(est),
      Q75 = quantile(est, 0.75),
      Q90 = quantile(est, 0.90),
      max = max(est),
      
      IQR = IQR(est),
      
      median_percent_difference = median(proportion_different) * 100,
      .groups = "drop"  # Ensures ungrouped data in the output
    )
}

###############################################################
# grouping_vars = c("design", "version", "variable")
beta_table_reduced <- summarize_betas(model_coefs)  

beta_table_reduced %>%
    select(design, version, n, Min=min, Q10, Q25, Q50=median, Q75, Q90, Max=max) %>%
  kable(caption = "reduced model. summary of each design's beta as a proportion of the reference and difference", 
        digits = 3) %>%
  kable_styling()
  
# for main paper & HEI appendix 
beta_table_reduced %>%
  select(design, version, n, Min=min, Q10, Q25, Q50=median, Q75, Q90, Max=max) %>%
  mutate_if(is.numeric, ~round(.,2)) %>%
  write.csv(., file.path(image_path, "beta_table_reduced_health_model_v1.csv"), row.names = F)


beta_table_extended <- summarize_betas(model_coefs_extended)  

beta_table_extended %>%
    select(design, version, n, median, Q25, Q75) %>%
  kable(caption = "extended model. summary of each design's beta as a proportion of the reference and difference", 
        digits = 3) %>%
  kable_styling()
...
beta_table_extended %>%
  select(design, version, n, Min=min, Q10, Q25, Q50=median, Q75, Q90, Max=max) %>%
  mutate_if(is.numeric, ~round(.,2)) %>%
  write.csv(., file.path(image_path, "beta_table_extended_health_model_v1.csv"), row.names = F)
###############################################################
# grouping_vars = c("design", "variable")
beta_table_reduced <- summarize_betas(model_coefs, grouping_vars = c("design", "variable"))  

beta_table_reduced %>%
  select(design, n, median, Q25, Q75) %>%
  kable(caption = "reduced model. summary of each design's beta as a proportion of the reference and difference", 
        digits = 3) %>%
  kable_styling()
  
write.csv(beta_table_reduced, file.path(image_path, "beta_table_reduced_health_model_v2.csv"), row.names = F)


beta_table_extended <- summarize_betas(model_coefs_extended, grouping_vars = c("design", "variable"))  
...
write.csv(beta_table_extended, file.path(image_path, "beta_table_extended_health_model_v2.csv"), row.names = F)
...
beta_summary_plot <- function(dt,
                              true_betas.,
                              variable.="Total|10-420",
                              facet_label="Reduced Health Model",
  ...
  p <- dt %>%
    ...
    mutate(version = gsub("Bus", "Business", version),
          ...
          version = factor(version, levels=c(version_levels2))
    ...
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = true_betas.$lower, ymax = true_betas.$upper,
           alpha = 0.1, fill = ref_model_col) +
    geom_hline(yintercept = true_betas.$est, linetype=2, col=ref_model_col,) +
    geom_hline(yintercept = 0, linetype=2, col="red") +

    scale_y_continuous(sec.axis = sec_axis(
      #trans=~(.-true_betas.$est)/-true_betas.$est,
      trans=~(.-true_betas.$est)/true_betas.$est,
                                         name= NULL,
                                         labels = scales::percent,
                                         breaks = y_lims_pct),
...
ggarrange(reduced_models,  extended_models, ncol = 1, common.legend = T, legend = "bottom") %>%
  annotate_figure(left = text_grob(expression(beta[m]), rot = 90, size = 18),
                  right = text_grob("% Difference from Reference", rot = -90, size = 18),
                  bottom = "Design")

ggsave(file.path(image_path, "beta_boxplots_main_reduced.png"), bg="white", #width = 12, height = 10
       width=12,#12, 
       height=12#10
       )
...
reduced_models_hei <- beta_summary_plot(dt=model_coefs, true_betas.=true_betas, facet_label = "Model 1") #Main Health Model

extended_models_hei <- beta_summary_plot(dt=model_coefs_extended, true_betas.=true_betas_extended, facet_label = "Model 2") #Extended Health Model

ggarrange(reduced_models_hei,  extended_models_hei, ncol = 1, common.legend = T, legend = "bottom") %>%
  annotate_figure(left = text_grob(expression(beta), rot = 90, size = 16),
                right = text_grob("% Difference from Reference", rot = 270, size = 16))

ggsave(file.path(image_path, "HEI Report", "beta_boxplots_main_reduced.png"),bg = "white", width = 12, height = 10)

################################################################################################
# SI - other pollutants
#y_lims_sensitivity <- range(model_coefs_extended$est)
y_lims_sensitivity <- range(config$health_plot$min_y_ci, config$health_plot$max_y_ci)

y_lims_sensitivity_pct <- seq(-2,2, 2)
...
lapply(c("10-420 nm", "10-100 nm", "20-1,000 nm", "NO2 (ppb)"), 
  ...
  p <- model_coefs_extended %>%
    ...
    # geom_hline(yintercept = ref_conc, linetype=1) +
    # geom_hline(yintercept = 0, linetype=2, alpha=0.2) +
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = true_betas2$lower, ymax = true_betas2$upper,
           alpha = 0.1, fill = ref_model_col) +
    geom_hline(yintercept = true_betas2$est, linetype=2, col=ref_model_col,) +
    geom_hline(yintercept = 0, linetype=2, col="red") +
    
    scale_y_continuous(sec.axis = sec_axis(
      #trans=~(.-ref_conc)/-ref_conc, 
      trans=~(.-ref_conc)/ref_conc, 
      name=NULL, labels = scales::percent, breaks = y_lims_sensitivity_pct
  ...
  ggarrange(plotlist = ., ncol = 1, common.legend = T, legend = "bottom") %>%
  annotate_figure(right = "% Difference from Reference", 
                   
                   left = text_grob(expression(beta[m]), 
                                   rot = 90, size = 14),
                  )
  
ggsave(file.path(image_path, "SI", "beta_boxplots.png"), bg="white", width = 10, height = 12)
...
```{r}
# presentation example of results for reduced model
...
beta_summary_plot_sample <- function(dt=model_coefs,
                              true_betas.= true_betas,
                              variable.="Total|10-420",
                              facet_label="",
                              reference_text_offset=0,
  ...
  p <- dt %>%
    filter(grepl(variable., variable),
           !design %in% c("All Data", "Unbalanced Visits"),
           !grepl("Adj", version)
           ) %>%
    mutate(version = gsub("Bus", "Business", version),
           version = gsub("Adj 1", "\nTemporally\nAdjusted", version),
    ...
    ggplot(aes(x=version, fill=ref)) +
    geom_boxplot(stat="identity", aes(ymin=Q10, lower=Q25, middle=Q50, upper=Q75, ymax=Q90)) +
    facet_grid(~design, scales="free_x", switch="both") +
    ...
    theme(
      ...
      legend.title = element_text(size = 14),    # Legend title (if used)
      strip.text = element_text(size = 14)       # Facet label text
    ) +

    labs(y = expression(beta),  
         x = "Restricted Sampling Monitoring Design", 
       fill= NULL)
...
beta_summary_plot_sample()

ggsave(file.path(image_path, "Other", "presentation", "beta_boxplots_reduced_sample.png"), bg="white", width = 9, height = 5)


```


### presentation with measurement error beta

comparison of corrected & uncorrected beta & CIs along with some example campagins 

```{r}
sample_design_betas <- model_coefs  %>%
    filter(grepl("nstot", model),
           # select a few example versions: 4 visits, 1 season, rh, bh
           grepl("v04|fewhrs_bh_", model)) %>%
    mutate(version = gsub("Bus", "Business", version),
           version = factor(version, levels=version_levels2),
           design = case_when(
             design== "Fewer Visits" ~ "12 Visits",
             design== "Fewer Hours" & version== "Business" ~ "12 Visits (Business Hours)",
             TRUE ~design
           ),
           type="Reduced Monitoring"
           ) %>% 
  
  select(design, version, campaign, est, lower, upper, type)

uncorrected_beta <- true_betas %>%
  filter(variable=="10-420 nm") %>%
  select(est, lower, upper) %>%
  mutate(design="Uncorrected",
         campaign=1,
...
corrected_beta <- me %>%
  mutate(
    lower = adjusted_beta - z_value*adjusted_SE,
    upper = adjusted_beta + z_value*adjusted_SE
    ) %>%
  select(est = adjusted_beta, lower, upper) %>%
  mutate(design = "Corrected",
         campaign=1,
         type="Measurement Error"
         )
 
example_betas <- bind_rows(sample_design_betas,
                           uncorrected_beta) %>%
  bind_rows(corrected_beta) %>%
  mutate(design = factor(design, levels=c("Corrected",
                                          "Uncorrected",
                                          "12 Visits",
                                          "12 Visits (Business Hours)"))) %>%
  droplevels()

# plot
example_betas %>%
  ggplot(aes(x=campaign, col=design)) +
  facet_grid(cols=vars(type, design), scales = "free_x",drop = T)+
  
  
  geom_hline(yintercept = 0, linetype=2, alpha=0.2) +
  geom_rect(inherit.aes = F, data=select(corrected_beta, -c(design, type)), aes(ymin = lower, ymax = upper, xmin=-Inf, xmax=Inf), alpha=rectangle_alpha, fill=ref_model_col) +
  geom_hline(data=select(corrected_beta, -c(design, type)),
             aes(yintercept = est),
             col=ref_model_col, linetype=2) +
  geom_pointrange(aes(y=est, ymin=lower, ymax=upper), #size=0.4,
                  position=position_dodge(width=0.7), show.legend = F) +
  scale_y_continuous(sec.axis = sec_axis(
    trans=~(.-corrected_beta$est)/corrected_beta$est,
    name= NULL,
    labels = scales::percent,
    # ~6%, 12%, 62% bias
    breaks = c(-1, -0.6, -0.15, #-0.06, 
               0, 0.5)
    )) +
   theme(
  ...
  scale_color_manual(values = c("#009E73", #ref_model_col,
                                "#A6D854",
                                "#E78AC3",
                                "#FC8D62"
  ...
  labs(y = expression(beta ~ " (95% CI)"), 
       #x= NULL,#"Campaign", 
       shape=NULL, col=NULL)

ggsave(file.path(image_path, "Other", "presentation", "beta_CI_sample.png"), bg="white", width = 10, height = 4)
...
reduced models are 'primary' models & no NO2
 
```{r}
# SI - other pollutants
y_lims <- range(config$health_plot$min_y_ci, config$health_plot$max_y_ci)
y_lims_sensitivity_pct <- seq(-2,2, 1)

print("extended health models")
# x="20-1,000 nm"
lapply(c("10-420 nm", "10-100 nm", "20-1,000 nm"), 
       function(x) {
  ...
  p <- model_coefs %>%
    ...
    scale_y_continuous(sec.axis = sec_axis(trans=~(.-ref_conc)/-ref_conc, name=NULL, labels = scales::percent, 
                                           breaks = y_lims_sensitivity_pct
                                           ), position="left", 
                       limits = y_lims) +
  labs(y=NULL, x = "Design", fill=NULL)
  ...
  ggarrange(plotlist = ., ncol = 1, common.legend = T, legend = "bottom") %>%
  annotate_figure(right = "% Different from Reference", 
                  left = "Health Point Estimate")
  
ggsave(file.path(image_path, "HEI Report", "Appendix", "beta_boxplots.png"), bg="white", width = 10, height = 12)
...
ap_age_equivalents %>%
  ...
  kable(caption = "Estimated associations between air pollutants (PNC [1,900 pt/cm3], NO2 [3 ppb]) and one year of aging with baseline CASI-IRT score based on all-data air pollution exposure models. Air pollution associations are scaled relative to the age association to contextualize their effects.",
        ...
        col.names = c("Model", "Pollutant", "Air Pollution Association", "Age Association", "Air Pollution Month Equivalent",
                      "Lower Equivalent", "Upper Equivalent"
                      )
        ) %>%
  kable_styling()
