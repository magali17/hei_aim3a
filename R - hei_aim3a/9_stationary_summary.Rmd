...
coef_all <- readRDS(file.path(output_data_path, "model_coefs_all.rda")) 
coef_r_all <- readRDS(file.path(output_data_path, "models_r_all.rda")) 
coef_lcm_all <- readRDS(file.path(output_data_path, "models_lcm_all.rda")) 
...
lapply(c("10-420 nm", "10-100 nm", "20-1,000 nm"), function(x) {
  ...
  p <- model_coefs %>%
    ...
    geom_hline(yintercept = 0, linetype=2, alpha=0.2) +
    scale_y_continuous(sec.axis = sec_axis(trans=~./ref_pnc, name=NULL, labels = scales::percent), position="right") +
  ...
  ggarrange(plotlist = ., ncol = 1, common.legend = T, legend = "bottom") %>%
  annotate_figure(left = "Health Point Estimate (% of Reference Model)",
