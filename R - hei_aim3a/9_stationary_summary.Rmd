...
cw <- read.csv(file.path(dt_path, "model_cw.csv")) %>%
  filter(design %in% c("full", "fewer total stops") |
           # this sensitivity analysis doesn't have a temporal adjustment
           design == "fewer hours" & grepl("ns10100", model)
           ) %>%
...
model_coefs_sensitivity <- readRDS(file.path(output_data_path, "model_coefs_sensitivity.rda")) %>%
  # make sure the latest model performances are used
  select(-c(out_of_sample, reference, RMSE, contains("R2"))) %>% 
  filter(model %in% unique(cs$model), #model_eval$model,
         # drop old/duplicate models
...
predictions <- readRDS(file.path(dt_path, "UK Predictions", 
  ...
  mutate(adjusted = ifelse(grepl("Adj", version), "Adjusted", "Unadjusted"),
         ...
         version = gsub(" Adj 1|Adj 2", "", version)) %>%
...
print("median predicted exposure difference for each sampling design vs CASI-IRT") 
...
lapply(design_labels, function(x) {
  ...
  prediction_comparison %>%
    ...
    ggplot(aes(x=casi_irt, col=version, y=prediction_diff)) + 
    facet_wrap(~design) + 
    ...
    ylim(min(prediction_comparison$prediction_diff), max(prediction_comparison$prediction_diff)) +
    scale_fill_continuous(type = "viridis") +
    labs(x=NULL, y=NULL)
  
  }) %>%
  ggarrange(plotlist = ., align = "hv") %>%
  annotate_figure(left = "Difference in Predicted PNC Exposure Relative to the All-Data Campaign", 
...
# for NS
# dt=model_coefs_sensitivity
# true_betas.=true_betas_sensitivity

beta_summary_plot <- function(dt, true_betas.) {
  true_betas. <- filter(true_betas., grepl("Total|10-420", variable)) %>%
  ...
  if(true_betas.$est>0){
    p <- p + scale_y_continuous(sec.axis = sec_axis(trans=~(.-true_betas.$est)/true_betas.$est, name="Health Point Estimate (% Different from Reference)", labels = scales::percent), position="right") 
    }
