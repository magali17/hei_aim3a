...
annual_comparison <- bind_rows(annual_unadjusted, annual_adj2) %>%
...
annual_comparison %>%
  pivot_wider(names_from=version, values_from=annual_mean) %>%
  
  pivot_longer(cols=vars(contains("temp_adj")), name_to="adjusted", values_to="value") %>%
