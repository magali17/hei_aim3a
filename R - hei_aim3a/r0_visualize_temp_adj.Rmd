...
# --> TO DO: UPDATE FILE PATHS TO LATEST FILES FROM r0_temporal_adjustment.R
...
annual_adj2_main0 <- filter(readRDS(file.path(dt_pt2, "site_avgs_uw_adj_no_hwy.rds")), background_adj == main_bg) %>% 
  select(-background_adj)
# dim(annual_adj2_main0)
...
# dim(temporal_adjustments1)

## unadjusted annual averages
annual_unadjusted <- bind_rows(readRDS(file.path("data", "onroad", "annie", "v2", "nonspatial_site_avgs.rds")),
                    readRDS(file.path("data", "onroad", "annie", "v2", "cluster_site_avgs.rds"))) %>%
  filter(grepl("business", version),
         cluster_type %in% unique(temporal_adjustments1$cluster_type) #drops cluster3
         )
# dim(annual_unadjusted)

## combine unadjusted and temporally-adjusted annual averages
annual_comparison <- bind_rows(temporal_adjustments1, annual_adj2_main0) %>%
  rename(annual_mean_adj = annual_mean) %>%
  left_join(select(annual_unadjusted, -c(version, cluster_value)))

dim(annual_comparison) 

  # 
  # bind_rows(annual_unadjusted) %>%
  # mutate(version = gsub("business hours", "business", version),
...
time_series_plot <- function(runs) {
  ...
  bind_rows(all, no_highways) %>% 
    ...
    mutate(UFP = gsub("ufp", "UFP", UFP),
           UFP = gsub("lta", "LTA", UFP),
           UFP = gsub("_", " ", UFP),
           road_types = gsub("_", " ", road_types),
           ) %>%
...
ggsave(file.path(image_path, "SI", "time_series_sample.png"), width = 12, height = 8)
...
# --> TO DO - why are some temp adj missing?? old file?

```{r}

annual_comparison %>%
  pivot_wider(names_from=version, values_from=annual_mean) %>% View()
  
  pivot_longer(cols=contains("temp_adj"), #names_to=adjusted, values_to=value
               ) #%>%
  
  ggplot(aes(x=business, y=value, col=version)) + 
  facet_wrap(~adjusted+visits) + 
