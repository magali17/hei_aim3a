...
if(!file.exists(file.path(dt_out, "TEMP_bh_visits.rda"))) {
  ...
  saveRDS(visits, file.path(dt_out, "TEMP_bh_visits.rda"))
  rm(list=c("v1", "v2"))
} else {
    visits <- readRDS(file.path(dt_out, "TEMP_bh_visits.rda"))
  }
...
if(!file.exists(file.path(dt_pt, "TEMP_road_dt.rda"))) {
  ...
  # apply(road_dt0, 2, function(x) any(is.na(x)))
  ...
  # apply(road_dt, 2, function(x) sum(is.na(x)))
  
  rm(time_series)
  
  saveRDS(road_dt, file.path(dt_pt, "TEMP_road_dt.rda"))
  
  # 25% of second data are missing here (e.g., from stops)
  # (nrow(time_series) - nrow(road_dt0))/nrow(time_series)
} else {
...
if(!file.exists(file.path(dt_pt, "underwrite_temp_adj_all_1s_data.rda"))) {
  ...
  road_dt <- lapply(windows, function(w) {
  ...
  # --> COULD ADD OVERNIGHT ADJUSTMENT SINCE THERE IS NO MM DATA DURING THESE TIMES
...
# apply(underwrite_adj, 2, function(x) sum(is.na(x)))
...
##################################################################################################
# EXAMPLE - visualize hourly adjustments
adj_lvls <- apply(expand.grid(paste0("hr", c(1:3)), 
                              paste0("_pct", c(1, 5, 10, 20))), 
                  1, paste, collapse="")
