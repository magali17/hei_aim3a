...
model_coefs <- readRDS(file.path(output_data_path, "model_coefs_lcm.rda")) %>%
  mutate(variable= ifelse(grepl("no2", model), "NO2", "PM2.5"),  
...
```{r}
ref_beta <- model_coefs$est[model_coefs$model=="no22_1" & model_coefs$model_type=="ST"]
...
model_coefs %>%
  ...
  mutate(method = str_wrap(method, width = 22)) %>%
