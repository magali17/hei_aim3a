...
rbind(t1_a, t1_b) %>%
  ...
  kable(caption = "Onroad sampling designs. Unbalanced sampling occurs when sites receive a different number of visits, sampled on a log-normal distribution with median of 4 or 12 visits. Balanced sampling occurs when all sites receive the same number of visits, either 4 or 12") %>% 
  kable_styling()
...
lapply(spatial_names, function (x) {
  ...
  geom_text(data=prop_sign_temp, aes(x=-Inf, y=0.01, label = text_label), 
            ...
            size=3) +
    geom_text(aes(x=Inf, y=-0.020, label = "Best Health Estimate"),
