...
annual_fixed_seasons <- readRDS(file.path(dt_path, "fixed_season_annual.rda"))  %>%
  select(-no2)
## this is only for total NS (drops P-TRAK used in sensitivity)
...
annual_trainint_set <- readRDS(file.path(dt_path, "annual_training_set.rda")) %>%
  filter(design %in% c("full", "fewer total stops"#, "fewer hours" #using this since it has p-trak estimates
                       )) %>%
  select(-no2, -spatial_temporal) %>%
  bind_rows(annual_site_type) %>%
  bind_rows(annual_fixed_seasons) %>% bind_rows(annual_temp_adj)
...
#for boxplots
bp_min <- 0.025
print(paste("Qmin is", bp_min))
bp_max <- 0.975
print(paste("Qmax is", bp_max))
...
## --> temp adj is only for NS total - fix? 
...
annual_avgs %>%
  #filter(design != "Site Type") %>%
  ggplot(aes(x=version, y=value, col=interaction(instrument, variable, sep = " "))) + 
  #ggforce::facet_row(~design, scales="free_x", space="free") + 
  facet_grid(~design, scales="free_x", space="free") + 
  #facet_wrap(~design, scales="free_x",  nrow = 2) + 
...
ggsave(file.path(image_path, "SI", "site_averages.png"), width = 10, height = 5)
...
lapply(sensitivity_pollutant_labels, function(x) {
  ...
  filter(variable== x) %>%
    ...
    facet_wrap_equal(~design+version, ncol=2) + 
...
# --> temp adj improves rush hour designs?
