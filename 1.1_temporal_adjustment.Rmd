...
```{r}
# --> NOTE: Documentation says that for factors, a main effect for dow should probably been included (this is automatically generated like with lm())
...
plot(lm1)
##################################################################
# QC check - Documentation says that for factors, a main effect for dow should probably been included (this is automatically generated like with lm()). Checking that  the results are similar.

set.seed(2)
# use cyclic spline?? mgcg::s(x, bs = "ad", xt = list(bs = "cc")) 
lm1_alt <-  calibration_dt %>%
  mutate(hour = as.numeric(as.character(hour))) %>%
  gam(data=., formula=log(ns_total_conc)~log(no2) + dow + 
                                                    s(hour,
                                                      bs="cc", #cyclic cubic regression spline
...
print("alternative with dow also as a main effect")
par(mfrow = c(3, 3))
plot(lm1_alt)
par(mfrow = c(1, 1))                                                       
##################################################################
...
calibration_dt <- calibration_dt %>%
  mutate(ufp_prediction = predict(lm1) %>% exp(),
         ufp_prediction_alt = predict(lm1_alt) %>% exp(),
         residual = ns_total_conc - ufp_prediction,
         residual_alt = ns_total_conc - ufp_prediction_alt,
...
# time series 
## --> note, if I use y=residual_alt, the temporal trend is slightly reduced 
calibration_dt %>%
...
predicted_ufp_alt <- nox %>%
  mutate(hour = as.numeric(as.character(hour))) %>% 
  predict(lm1_alt, newdata=.) %>% 
  exp()
...
nox <- nox %>%
  mutate(predicted_ufp = predicted_ufp,
         ...
         #results we would have gotten with DOW as a main effect
         predicted_ufp_alt = predicted_ufp_alt,
         )
...
```{r}
# compare site-level estimates vs prediction error
#
# band <- 0.3
# 
# adjusted_annual_avg %>%
#   left_join(all_data_annual) %>% 
#   select(design, version, campaign, location, unadjusted_obs=ns_total_conc, adjusted_obs=ns_total_conc_adj_no2lm_winsorize, true_obs=ns_total_conc_all_data) %>%
#   pivot_longer(cols = c(contains(c("adjusted")))) %>% 
#   mutate(error = value-true_obs) %>%
#   
#   ggplot(aes(x=true_obs, y=value, col=name)) + 
#   geom_point(alpha=0.3) +
#   #facet_wrap(~name) +
#   geom_abline(intercept = 0, slope = c(1-band, 1+band), linetype=3, alpha=0.5) +
