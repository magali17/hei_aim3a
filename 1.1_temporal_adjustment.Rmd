...
# linearly interpollate. note that 3 days have 16, 22, and 24 missing values
nox <- nox %>%
  mutate(no2_interpolated = ifelse(is.na(no2), TRUE, FALSE),
          no2 = na.approx(no2, #maxgap = 12, 
                                       #na.rm = F
                                       )) 
...
```{r}
# save data
saveRDS(adjusted_annual_avg, file.path(dt_path, "campaign visit samples", "temporal_adjustment.rda") )
